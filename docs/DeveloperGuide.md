...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `StaffListPanel`, `CustomerListPanel`, `DrinkListPanel`, `StatusBarFooter` etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class which captures the commonalities between classes that represent parts of the visible GUI.
...
* depends on some classes in the `Model` component, as it displays `Staff`, `Customer`, and `Drink` objects residing in the `Model`.
