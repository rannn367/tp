...
public class EditCustomerCommandParserTest {
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_ALEX + TAG_DESC_HUSBAND
                + EMAIL_DESC_ALEX + ADDRESS_DESC_ALEX + NAME_DESC_ALEX + TAG_DESC_FRIEND;

        EditCustomerDescriptor descriptor = new EditCustomerDescriptorBuilder().withName(VALID_NAME_ALEX)
    ...
    public void parse_someFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_ALEX + EMAIL_DESC_ALEX;

        EditCustomerDescriptor descriptor = new EditCustomerDescriptorBuilder().withPhone(VALID_PHONE_ALEX)
                .withEmail(VALID_EMAIL_ALEX).build();
        EditCustomerCommand expectedCommand = new EditCustomerCommand(targetIndex, descriptor);
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        EditCustomerDescriptor descriptor = new EditCustomerDescriptorBuilder().withName(VALID_NAME_ALEX).build();
        EditCustomerCommand expectedCommand = new EditCustomerCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // phone
        userInput = targetIndex.getOneBased() + PHONE_DESC_ALEX;
        descriptor = new EditCustomerDescriptorBuilder().withPhone(VALID_PHONE_ALEX).build();
        expectedCommand = new EditCustomerCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // email
        userInput = targetIndex.getOneBased() + EMAIL_DESC_ALEX;
        descriptor = new EditCustomerDescriptorBuilder().withEmail(VALID_EMAIL_ALEX).build();
        expectedCommand = new EditCustomerCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // address
        userInput = targetIndex.getOneBased() + ADDRESS_DESC_ALEX;
        descriptor = new EditCustomerDescriptorBuilder().withAddress(VALID_ADDRESS_ALEX).build();
        expectedCommand = new EditCustomerCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
